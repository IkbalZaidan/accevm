(function ($) {
  ...
  if ($('.service-style1-tab').length) {
    $('.service-style1-tab .tabs-button-box .tab-btn-item').on('click', function (e) {
      console.log("service-style1-tab");
      e.preventDefault();
      ...
      if ($("#polyglot-language-options").length) {
        $('#polyglotLanguageSwitcher').polyglotLanguageSwitcher({
          ...
          onChange: function (evt) {
            try {
              const selectedLanguage = evt.selectedItem; // Adjust if evt has a different structure
              if (!selectedLanguage) {
                throw new Error("No selected language found in the event.");
              }
              ...
              if (window.i18n) {
                window.i18n.changeLanguage(selectedLanguage)
                  .then(() => {
                    ...
                    console.log(`Language switched to: ${selectedLanguage}, direction: ${direction}`);
                ...
                throw new Error("i18n is not available globally.");
              }
            } catch (error) {
              console.error("Error in language switcher onChange handler:", error);
            }
          },
        });
      }
